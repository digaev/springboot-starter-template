const { writeFileSync } = require('fs');
const { join } = require('path');
const rimraf = require('rimraf');

const MiniCssExtractPlugin = require('mini-css-extract-plugin');
const OptimizeCSSAssetsPlugin = require('optimize-css-assets-webpack-plugin');
const UglifyJsPlugin = require('uglifyjs-webpack-plugin');

const DEV_MODE = process.env.NODE_ENV === 'development';

// Output directory for compiled assets (automatically created and removed)
const ASSETS_DIR = join(__dirname, 'src', 'main', 'resources', 'public', 'assets');

// Source directory
const WEBINF_DIR = join(__dirname, 'src', 'main', 'webapp', 'WEB-INF');

// Directory for generated JSP templates (see `src/main/webapp/WEB-INF/tags/layout.tag`)
const JSP_DIR = join(WEBINF_DIR, 'views');

// Shouldn't ends with '/' (use empty string '' for root url)
const PUBLIC_URL = '/assets';

// Generate JSP
class JSPGenerator {
  // eslint-disable-next-line class-methods-use-this
  apply(compiler) {
    const views = join(WEBINF_DIR, 'views');

    compiler.hooks.beforeCompile.tap('JSPGenerator', () => {
      rimraf.sync(ASSETS_DIR);
    });

    compiler.hooks.done.tap('JSPGenerator', (stats) => {
      const app = `${PUBLIC_URL}/app${DEV_MODE ? '' : `-${stats.hash}`}`;

      writeFileSync(
        join(JSP_DIR, '_script.jsp'),
        `<%-- This file was generated by JSPGenerator on ${new Date()} --%>\n\n`
        + `<script src="${app}.js"></script>\n`,
        'utf8',
      );

      writeFileSync(
        join(JSP_DIR, '_style.jsp'),
        `<%-- This file was generated by JSPGenerator on ${new Date()} --%>\n\n`
        + `<link href="${app}.css" rel="stylesheet" type="text/css">\n`,
        'utf8',
      );
    });
  }
}

module.exports = {
  mode: DEV_MODE ? 'development' : 'production',
  context: join(WEBINF_DIR, 'assets'),
  entry: {
    app: ['./js/app.js', './css/app.styl'],
  },
  output: {
    filename: DEV_MODE ? '[name].js' : '[name]-[hash].js',
    path: join(ASSETS_DIR),
    publicPath: `${PUBLIC_URL}/`,
  },
  devtool: DEV_MODE ? 'source-map' : undefined,
  module: {
    rules: [{
      test: /.js$/,
      use: [{
        loader: 'babel-loader',
        options: {
          presets: ['@babel/preset-env'],
        },
      }],
    }, {
      test: /.css$/,
      use: [
        MiniCssExtractPlugin.loader,
        'css-loader',
      ],
    }, {
      test: /.styl$/,
      use: [
        MiniCssExtractPlugin.loader,
        'css-loader',
        'stylus-loader',
      ],
    }, {
      test: /\.(png|jpg|svg|ttf|eot|woff|woff2)$/,
      use: [{
        loader: 'file-loader',
        options: {
          name: '[name].[ext]',
        },
      }],
    }],
  },
  plugins: [
    new JSPGenerator(),
    new MiniCssExtractPlugin({
      filename: DEV_MODE ? '[name].css' : '[name]-[hash].css',
    }),
  ],
  optimization: {
    minimizer: [new UglifyJsPlugin(), new OptimizeCSSAssetsPlugin()],
  },
};
